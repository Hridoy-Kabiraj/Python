...
BC_type = ["hard", "soft"][0]  # [0] → picks the element at index 0

# Defining square domain boundary

# boundary for x
#Checks if the point is on the left vertical boundary (x = 0)
def boundary_left(x, on_boundary):
    return on_boundary and dde.utils.isclose(x[0], 0.0)

# Checks if the point is on the right vertical boundary (x = 1)
def boundary_right(x, on_boundary):
    return on_boundary and dde.utils.isclose(x[0], 1)

# boundary for y
# Checks if the point is on the top horizontal boundary (y = 1)
def boundary_top(x, on_boundary):
    return on_boundary and dde.utils.isclose(x[1], 1.0)

# Checks if the point is on the bottom horizontal boundary (y = 0)
...
def func(x):
    ...
    # E_xx = dux/dx and E_yy = duy/dy and Exy = 0.5*(dux/dy + duy/dx) 
    E_xx = -2 * np.pi * np.sin(2*np.pi*x[:, 0:1]) * np.sin(np.pi*x[:, 1:2])
    E_yy = np.sin(np.pi*x[:, 0:1]) * Q * x[:, 1:2]**3 
    E_xy = 0.5 * (np.cos(2*np.pi*x[:, 0:1]) * np.pi * np.cos(np.pi*x[:, 1:2])
                  + np.pi * np.cos(np.pi*x[:, 0:1]) * Q * x[:, 1:2]**4 / 4)
    
    Sxx = (lmbd + 2*mu) * E_xx + lmbd * E_yy
    Syy = (lmbd + 2*mu) * E_yy + lmbd * E_xx
    Sxy = 2*mu*E_xy

    return np.hstack((ux, uy, Sxx, Syy, Sxy))

# Soft boundary conditions
ux_top_bc = dde.icbc.DirichletBC(geom, lambda x: 0, boundary_top, component=0)  # component = index of the output variable in the NN, 0 = ux
...
# x: the coordinates [x, y]
# f: the raw NN outputs [ux, uy, Sxx, Syy, Sxy]
def hard_BC(x, f):
    Ux = f[:, 0] * x[:, 1] * (1 - x[:, 1])
...
# Here, The trick is multiplying the NN outputs by factors like x[:, 0] * (1 - x[:, 0]) or x[:, 1] * (1 - x[:, 1])
# At the boundary (x=0 or x=1, or y=0 or y=1), those terms go to zero, forcing the NN’s prediction to match the BC exactly
# Inside the domain, the NN still has flexibility via f
# Ux = f[:, 0] * x[:, 1] * (1 - x[:, 1]), Forces Ux = 0 at y=0 and y=1. So u_x automatically satisfies Dirichlet BCs on the top/bottom boundaries
